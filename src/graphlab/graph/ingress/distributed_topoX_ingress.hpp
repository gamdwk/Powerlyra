...
#ifndef GRAPHLAB_DISTRIBUTED_TOPOX_INGRESS_HPP
#define GRAPHLAB_DISTRIBUTED_TOPOX_INGRESS_HPP
...
namespace graphlab {
  ...
  public:
    ...
    dc_dist_object<distributed_topoX_ingress> topoX_rpc;
    ...
    std::vector<edge_buffer_record> topoX_edges;

    /* ingress exchange */
    buffered_exchange<edge_buffer_record> topoX_edge_exchange;
    buffered_exchange<vertex_buffer_record> topoX_vertex_exchange;
	...
	size_t count[10];         //记录各节点现有的分配到的边的数目，如何分布式？
  ...
  public:
    distributed_topoX_ingress(distributed_control& dc,
		graph_type& graph, size_t threshold = 100, size_t theta = 100,size_t etheta=1000,size_t ceng=1) :
        base_type(dc, graph), topoX_rpc(dc, this),
		graph(graph), threshold(threshold), theta(theta), etheta(etheta),ceng(ceng),
#ifdef _OPENMP
        topoX_edge_exchange(dc, omp_get_max_threads()),
		vht_exchange(dc, omp_get_max_threads()),
        topoX_vertex_exchange(dc, omp_get_max_threads())
#else
        topoX_edge_exchange(dc),
        topoX_vertex_exchange(dc)
#endif
    {
      /* fast pass for standalone case. */
      standalone = topoX_rpc.numprocs() == 1;
      topoX_rpc.barrier();
    } // end of constructor

    ~distributed_topoX_ingress() { }
    ...
    void add_edge(vertex_id_type source, vertex_id_type target,
		...
		size_t numproc = topoX_rpc.numprocs();
		const edge_buffer_record record(source, target, edata);
		const size_t sh = source%numproc;
		const size_t th = target%numproc;
#ifdef _OPENMP
		if (sh!=th)
			topoX_edge_exchange.send(sh, record, omp_get_thread_num());
		topoX_edge_exchange.send(th, record, omp_get_thread_num());
#else
		if (sh != th)
			topoX_edge_exchange.send(sh, record);                  //把一条边发给source、target所在的两个节点下，如果位于同一节点，只发送一次
		topoX_edge_exchange.send(th, record);
#endif
    ...
    void add_vertex(vertex_id_type vid, const VertexData& vdata) {
      ...
      const procid_t owning_proc = standalone ? 0 :
        vid % topoX_rpc.numprocs();
#ifdef _OPENMP
      topoX_vertex_exchange.send(owning_proc, record, omp_get_thread_num());
#else
      topoX_vertex_exchange.send(owning_proc, record);
#endif
    } // end of add vertex
	...
	void set_queue(const size_t& v, const size_t& owning,const size_t& vp,const size_t& topoXcount){
		if (flaght[v] == 0){
			point_lock.lock();
			p_queue.push(v);
			if (fht[v] == -1){
				fht[v] = owning;
				countht[v] = topoXcount+1;
			}
			point_lock.unlock();
		}
		if (flaght[vp] == 0){
	...
	void add_queue(const size_t& v,const size_t& owning,const size_t& vp,const size_t& topoXcount){
		size_t owningmachine = v%topoX_rpc.numprocs();
		if (owningmachine != topoX_rpc.procid())
			topoX_rpc.remote_call(owningmachine, &distributed_topoX_ingress<VertexData, EdgeData>::set_queue, v, owning, vp,topoXcount);
		else
			set_queue(v, owning,vp,topoXcount);
	}
    ...
    void finalize() {
      ...
      size_t nprocs = topoX_rpc.numprocs();
	  size_t cnodes = nprocs / ceng;
      procid_t l_procid = topoX_rpc.procid();
      size_t nedges = 0;

      topoX_rpc.full_barrier();

      if (l_procid == 0) {
        memory_info::log_usage("start finalizing");
        logstream(LOG_EMPH) << "topoX finalizing ..."
      ...
      /*                                                                        */
      /**************************************************************************/
      topoX_edge_exchange.flush(); topoX_vertex_exchange.flush();

      /**
       * Fast pass for redundant finalization with no graph changes.
       */
      {
        size_t changed_size = topoX_edge_exchange.size() + topoX_vertex_exchange.size();
        topoX_rpc.all_reduce(changed_size);
        if (changed_size == 0) {
      ...
      /**************************************************************************/
      /*                                                                        */
      /*                       Prepare topoX ingress                           */
      /*                                                                        */
      /**************************************************************************/
      {
        edge_buffer_type edge_buffer;
        procid_t proc;
        nedges = topoX_edge_exchange.size();

        topoX_edges.reserve(nedges);
        if (standalone) { /* fast pass for standalone */
          proc = -1;
          while(topoX_edge_exchange.recv(proc, edge_buffer))
			  foreach(const edge_buffer_record& rec, edge_buffer){
				  if (rec.target%nprocs == l_procid){
					  topoX_edges.push_back(rec);
          ...
          while(topoX_edge_exchange.recv(proc, edge_buffer)) {
				...
				foreach(const edge_buffer_record& rec, edge_buffer){         //按目的点存储该边，并根据边构建iht和oht
 					if (rec.target%nprocs == l_procid){
						topoX_edges.push_back(rec);
						iht[rec.target].push_back(rec.source);
						if (rec.source%nprocs == l_procid)
          ...
          topoX_edge_exchange.clear();
          topoX_edge_exchange.barrier(); // barrier before reusing
#ifdef TUNING
          if(l_procid == 0) {
			  ...
			  if (!p_queue.empty()){
				  ...
				  while (!p_queue.empty()){
					  ...
					  if (flaght[p_point] == 0){
						  ...
						  procid_t min_proc = fht[p_point];

						  if (countht[p_point] > etheta){
							  int temp2 = count[0];
							  min_proc = 0;
		...
		//  vht_exchange.clear();
		  ...
		  for (size_t i = 0; i < topoX_edges.size(); i++){
			  procid_t owning_proc = 0;
			  edge_buffer_record& rec = topoX_edges[i];
			  if (dht[rec.target] <= threshold){
				  ...
				  if (owning_proc != l_procid){
					  for (int j = 0; j < ceng; j++){
						  topoX_edge_exchange.send(owning_proc%cnodes, rec);
					  }

					  // set re-sent edges as empty for skipping
					  topoX_edges[i] = edge_buffer_record();
					  --nedges;
				  }
          ...
          topoX_edge_exchange.flush();
#ifdef TUNING
          logstream(LOG_INFO) << "receive high-degree edges: "
                              << topoX_edge_exchange.size() << std::endl;
#endif
          proc = -1;
          while(topoX_edge_exchange.recv(proc, edge_buffer)) {
            foreach(const edge_buffer_record& rec, edge_buffer) {
      ...
      if(l_procid == 0) {
        memory_info::log_usage("prepare topoX finalizing done.");
        logstream(LOG_EMPH) << "prepare topoX finalizing. ("
                            << ti.current_time()
                            << " secs)"
                            << std::endl;
      }
      ...
      if(l_procid == 0) {
        memory_info::log_usage("topoX finalizing graph done.");
        logstream(LOG_EMPH) << "topoX finalizing graph. ("
                            << ti.current_time()
                            << " secs)"
                            << std::endl;
      }
    ...
    void set_degree_type() {
      ...
      std::vector<size_t> swap_counts(topoX_rpc.numprocs());

      swap_counts[l_procid] = high_master;
      topoX_rpc.all_gather(swap_counts);
      high_master = 0;
      ...
      if(l_procid == 0) {
        logstream(LOG_EMPH) << "topoX info: master ["
                            << high_master << " "
                            << low_master << " "
                            << (float(high_master)/(high_master+low_master)) << "]"
                            << std::endl;
        if ((high_mirror + low_mirror) > 0)
        logstream(LOG_EMPH) << "topoX info: mirror ["
                            << high_mirror << " "
                            << low_mirror << " "
     ...
     * extracting edges from topoX_edges instead of original edge_buffer
     */
    void modified_base_finalize(size_t nedges) {
      ...
      { // Add all the edges to the local graph
        ...
        foreach(const edge_buffer_record& rec, topoX_edges) {
          // skip re-sent edges
      ...
      // Setup the map containing all the vertices being negotiated by this machine
      {
        ...
        if (topoX_vertex_exchange.size() > 0) {
          vertex_buffer_type vertex_buffer; procid_t sending_proc(-1);
          while(topoX_vertex_exchange.recv(sending_proc, vertex_buffer)) {
            foreach(const vertex_buffer_record& rec, vertex_buffer) {
              ...
              if (distributed_topoX_ingress::vertex_combine_strategy
                && lvid < graph.num_local_vertices()) {
                distributed_topoX_ingress::vertex_combine_strategy(
          ...
          topoX_vertex_exchange.clear();
#ifdef TUNING
          logstream(LOG_INFO) << "base::#vert-msgs=" << topoX_vertex_exchange.size()
                              << std::endl;
          if(l_procid == 0) {
      ...
      if (!standalone) {
#ifdef _OPENMP
        buffered_exchange<vertex_id_type> vid_buffer(topoX_rpc.dc(), omp_get_max_threads());
#else
        buffered_exchange<vertex_id_type> vid_buffer(topoX_rpc.dc());
#endif
...
#pragma omp parallel
        ...
        if (!flying_vids.empty()) {
			logstream(LOG_INFO) << "procID= " << l_procid
			  << " #flying-own-nverts="
                              << flying_vids.size()
                              << std::endl;
      ...
      // TODO:  optimization for standalone
      {
        ...
        if (!first_time_finalize) {
          ...
          buffered_exchange<vertex_id_type> vset_exchange(topoX_rpc.dc());
        ...
        graphlab::graph_gather_apply<graph_type, vertex_negotiator_record>
            vrecord_sync_gas(graph,
                             boost::bind(&distributed_topoX_ingress::finalize_gather, this, _1, _2),
                             boost::bind(&distributed_topoX_ingress::finalize_apply, this, _1, _2, _3));
  ...
  }; // end of distributed_topoX_ingress
