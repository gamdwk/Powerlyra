 ...
 *
 *
 * 2017  implement synchronous engine of topoX
 *
 */



#ifndef GRAPHLAB_topoX_SYNC_ENGINE_HPP
#define GRAPHLAB_topoX_SYNC_ENGINE_HPP
...
namespace graphlab {
   ...
   * \ref graphlab::topoX_sync_engine::signal or
   * \ref graphlab::topoX_sync_engine::signal_all.  In either case all
   * machines should invoke signal or signal all at the same time.  Finally,
   * computation is initiated by calling the
   * \ref graphlab::topoX_sync_engine::start function.
   *
   * ### Example Usage
   *
   ...
   *   graphlab::topoX_sync_engine<pagerank_vprog> engine(dc, graph, clopts);
   *   engine.signal_all();
   *   engine.start();
